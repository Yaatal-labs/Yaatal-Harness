...
async fn handle_rss_feed(
    ...
    (
        StatusCode::OK,
        serde_json::json!({"success": true, "inserted": inserted}),
    )
}

async fn handle_github_trending(
    body: &Value,
    db: &sea_orm::DatabaseConnection,
    bot_id: &str,
) -> (StatusCode, Value) {
...
async fn handle_devto_sync(
    ...
    for article in &articles {
        ...
        let _reactions = article
            .get("positive_reactions_count")
            .and_then(|v| v.as_i64())
            .unwrap_or(0);
        let content_raw = article
            .get("description")
...
async fn handle_content_post(
    ...
    if title.is_empty() || content.is_empty() {
        return (
            StatusCode::BAD_REQUEST,
            serde_json::json!({"error": "title and content required"}),
        );
    }
    let title = sanitize(&truncate(&title, 500));
    let content = sanitize(&truncate(&content, 5000));
    ...
    if insert_post(db, &title, &content, "discussion", category, bot_id).await {
        ...
        (
            ...
            serde_json::json!({"error": "Failed to create post"}),
        )
    }
}
...
pub async fn n8n(
    ...
    let (status, resp) = match workflow_type {
        ...
        _ => (
            StatusCode::BAD_REQUEST,
            serde_json::json!({"error": "Unknown workflow type"}),
        ),
    };
