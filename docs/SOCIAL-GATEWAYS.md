...
| **SMS (via self-hosted httpSMS)** | ✅ inbound → webhook (Android phone as gateway, own SIM) | ✅ outbound via HTTP API through the same device | a ~$50 Android + local SIM + self-hosted httpSMS (AGPL-3.0, Go, 3.4k★) — **no platform gatekeeper at all** | **Built 2026-07-11** — Engine `services/sms.rs` + `/webhooks/sms` (JWT-verified per httpSMS's actual scheme, read from source); channel-preference routing for delivery codes (WA else SMS); founder-brief leg — the deepest-reach channel: feature phones, no data plan, no Meta. Transactional volumes only (carrier anti-spam caps bulk); AGPL stays isolated behind the HTTP service boundary; own SIM = in-country message residency. Fits the existing thin-adapter pattern (Engine service + webhook → `social_events`, platform `"sms"`); delivery codes + payment-retry links + OTP are the first uses |
